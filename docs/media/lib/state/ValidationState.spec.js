...
describe('ValidationState', () => {
    ...
    it('state.set sets properties', async () => {
        const state = await new ValidationState(async (v) => validate.min(v, 2), VALID_VALUE).set(INVALID_VALUE);
    ...
    it('set with no change returns same state', async () => {
        const state = new ValidationState(async (v) => validate.min(v, 2), INVALID_VALUE);
...
//# sourceMappingURL=ValidationState.spec.js.map